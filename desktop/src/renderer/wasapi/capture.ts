...
async function startCapture(): Promise<void> {
  try {
    ...
    if (sysAudioTracks.length === 0) {
      throw new Error(
        'System audio was not captured. On Windows, check Sound Settings → ' +
    ...
    mediaRecorder.addEventListener('error', (e) => {
      const message = (e as unknown as { error?: { message?: string } })?.error?.message ?? 'MediaRecorder error';
    ...
    cleanup();
    window.wasapiAPI.error((err as Error).message || String(err));
  }
}
