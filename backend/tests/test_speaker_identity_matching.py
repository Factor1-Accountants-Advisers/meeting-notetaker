...
class SpeakerIdentityMatchingTests(unittest.TestCase):
    ...
    def test_approved_expansion_match_is_named_when_not_invited(self):
        """Approved IN-79 candidates remain eligible even without a calendar invite."""
        segments = [segment("SPEAKER_00", 0, 5000)]
        ranges = [
            IdentityRange(
                start_ms=0,
        ...
        matched, participants, unknown_count = _apply_identity_ranges(
            segments,
            ranges,
            min_confidence=0.85,
        ...
        self.assertEqual(unknown_count, 0)
        self.assertEqual([(p.name, p.known) for p in participants], [("David F", True)])
        self.assertTrue(matched[0].speaker_known)
        self.assertEqual(matched[0].speaker, "David F")
        self.assertEqual(matched[0].speaker_source, "pyannote_voiceprint_expansion")
