...
class RestCallWatchTests(unittest.TestCase):
    ...
    def test_get_call_signals_gets_current_signals_and_validates_shape(self):
        client = self._client(SIGNALS)
        result = client.get_call_signals("token-value")

        self.assertEqual(self.seen["method"], "GET")
        self.assertEqual(
            self.seen["url"], "https://storage.example/api/v1/call-watches/current/signals"
        )
        self.assertEqual(self.seen["authorization"], "Bearer token-value")
    ...
    def test_get_call_signals_rejects_malformed_shapes(self):
        ...
        with self.assertRaises(StorageApiContractError):
            self._client({"nope": []}).get_call_signals("token-value")

    def test_delete_call_watch_deletes_current_watch(self):
        client = self._client(None)
        result = client.delete_call_watch("token-value")

        self.assertEqual(self.seen["method"], "DELETE")
        self.assertEqual(
            self.seen["url"], "https://storage.example/api/v1/call-watches/current"
        )
        self.assertEqual(self.seen["authorization"], "Bearer token-value")
    ...
    def test_delete_call_watch_tolerates_404_as_success(self):
        ...
        self.assertIsNone(client.delete_call_watch("token-value"))
    ...
    def test_rest_fails_closed_without_a_token(self):
        ...
        with self.assertRaises(StorageApiUnavailable):
            client.get_call_signals(None)
        with self.assertRaises(StorageApiUnavailable):
            client.delete_call_watch(None)
    ...
    def test_rest_maps_5xx_to_unavailable_and_network_errors_to_unavailable(self):
        ...
        with self.assertRaises(StorageApiUnavailable):
            client.get_call_signals("token-value")
        with self.assertRaises(StorageApiUnavailable):
            client.delete_call_watch("token-value")
...
class StubCallWatchTests(unittest.TestCase):
    ...
    def test_get_call_signals_returns_empty_list(self):
        result = self.client.get_call_signals(None)
        self.assertIsInstance(result, CallSignalsResponse)
        self.assertEqual(result.signals, [])

    def test_delete_call_watch_is_a_no_op(self):
        self.assertIsNone(self.client.delete_call_watch(None))
        self.assertIsNone(self.client.delete_call_watch("token-value"))
    ...
    def test_stub_fail_next_applies_to_mutating_call_watch_operations(self):
        ...
        self.assertEqual(self.client.get_call_signals(None).signals, [])

        self.client.fail_next = True
        with self.assertRaises(StorageApiError):
            self.client.delete_call_watch(None)
