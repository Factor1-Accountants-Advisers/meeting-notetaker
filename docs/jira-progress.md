...
- [x] 2026-07-10 — IN-64 reconciliation pass (evidence, no feature rework)
  ...
  - **IN-88 current scope (reconciled against David's failed live test):** nav remains Home/People/Settings only; there is no per-meeting calendar Record button; Settings retains Audio capture. The Home ad-hoc live-capture action and recording-screen Pause/Resume/Stop buttons have been removed. Existing-audio upload remains available. Automatic Graph-triggered recording, automatic stop, status display, and the scheduled-meeting "Extend 10 min" action remain.
  - **IN-120 newer tray-control scope:** while an automatic recording is active, the tray exposes Pause/Resume, Stop recording, and Extend 10 min. Commands cross a dedicated main-to-renderer IPC bridge; the renderer owns media capture and mirrors pause state to refresh the tray label. The tray controls intentionally do not restore Home or recording-screen manual controls.
  ...
  - **IN-123/IN-126 home cleanup verified:** "Meeting Title" header and "Upload recording" action remain; the manual "Start Recording" action, Invite button, and meeting-link field are absent in `HomeScreen.tsx`. People tab is retained for voiceprint verification/enrollment.
