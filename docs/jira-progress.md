...
- [ ] `IN-68` — Implement MS Graph meeting detection in Electron main process
  ...
  - Remaining before crossing out: interactive sign-in/token cache population, live/persisted calendar sync with tenant credentials, startup/resume polling policy, and live redacted payload smoke if credentials are available.
