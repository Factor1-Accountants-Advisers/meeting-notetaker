...
- [ ] IN-378 — Retrieve meeting-scoped voiceprints for speaker identification
  ...
  - Release gate: neither repo branch is pushed or deployed by this implementation session. Production does not yet expose the IN-378 POST operation, so the coordinated authenticated smoke remains pending the Storage API `main` deployment. Joseph accepted the bounded employee-trust model on 24 Jul 2026: the submitted meeting UUID/candidates remain authenticated caller assertions, consistent with manual/ad-hoc recordings whose participant lists are user-entered. Live Jira contains no requirement for server-verifiable meeting membership, so the production-readiness finding is not release-blocking; stronger object authorization is deferred unless a future Jira or security requirement introduces it. After deployment, use the exact desktop registration/scope and a read-only candidate request; never print token, OID, email, or voiceprint data. Keep the Jira item open until the smoke succeeds.
