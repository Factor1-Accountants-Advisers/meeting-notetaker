...
- [ ] IN-378 — Retrieve meeting-scoped voiceprints for speaker identification
  ...
  - Release gate: neither repo branch is pushed or deployed by this implementation session. Production does not yet expose the IN-378 POST operation, so the coordinated authenticated smoke remains pending the Storage API `main` deployment. A production-readiness review also identified that the Storage API has no authoritative meeting roster: the submitted meeting UUID/candidates are caller assertions, so any delegated employee could repeat bounded requests for known staff emails. Do not merge/deploy until Joseph either explicitly accepts that bounded employee-only risk for the initial release or approves a larger server-verifiable meeting-membership design (Graph OBO or a trusted server-side manifest). After deployment, use the exact desktop registration/scope and a read-only candidate request; never print token, OID, email, or voiceprint data. Keep the Jira item open until that decision and smoke succeed.
