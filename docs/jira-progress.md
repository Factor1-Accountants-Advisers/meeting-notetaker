...
## Windows communications audio routing and Bluetooth recovery (4 Aug 2026)

- Implemented the approved three-layer design: a Rust Core Audio observer for
  Windows console/communications capture and render defaults; an Electron
  supervisor with strict JSONL parsing, cached IPC state, one bounded restart,
  and Chromium fallback; and renderer routing/recovery on top of the existing
  stable mic bridge and segmented loopback recorders.
- Replaced the legacy implicit `micDeviceId` preference with versioned
  `follow_communications` (recommended/default) and `pinned` modes. Existing
  manual IDs are retained as the last choice but migrate to automatic routing.
  Friendly-name resolution is exact after case/whitespace normalization;
  missing or ambiguous matches fall back with an explicit reason.
- Fixed the field-reported pinned-device defect: a live fallback stream no
  longer counts as proof that the selected device is active. A Bluetooth mic
  that was absent at start and reappears replaces the fallback upstream of the
  stable recorder without restarting MediaRecorder.
- Native and browser notifications coalesce into one route generation.
  Capture-endpoint changes replace the mic source; render changes create
  pause-aware loopback segments. Mic and loopback silence watchdogs each make
  one forced recovery attempt per generation with a 30-second cooldown.
- Settings now shows the recommended/manual modes, current Windows
  communications microphone/output, Teams system-default guidance, and a
  default-vs-communications output warning. The recording screen names the
  active endpoints and distinguishes switching, fallback, mismatch, and
  post-recovery silence.
- Packaging: the helper is built by `build:audio-helper`, shipped outside ASAR
  at `resources/audio/notetaker-audio-endpoints.exe`, and included in Azure
...
- Verification: Rust tests **3 passed**; Graph, recording-control, capture,
  ...
  packaging verification passed; `release.yml` parsed as YAML; `git diff
  --check` passed. Backend discovery ran **324 tests with 1 failure**, the
  previously documented timing-sensitive
  `test_stub_serializes_concurrent_exports_for_one_meeting` collision (12 of
  15 history files), unrelated to audio routing.
- Commits: `e51f243`, `c157214`, `cbd6bf6`, `07151dd`, `c2e768f`, `4bf1ef7`,
  `46f1eb7`, `b684861` (plus design/plan commits `49cf6d8`, `93d03ea`,
  `1cd8dfd`).
- **Still open:** signed-installer Bluetooth/Teams hardware acceptance across
  the eight design scenarios. No Jira transition, production deployment, or
  live Teams/headset pass is claimed by this implementation session.
