...
- [ ] IN-476 — Cut desktop enrolment over to the authenticated Storage API using Entra object IDs
  ...
  - TDD evidence: `scripts/verify-storage-cutover.ts` first failed on missing policy/config exports, then passed after the pure request, OID, header, rollback, and public-default implementation. `backend/tests/test_central_enrolment.py` first failed on the missing OID route/config fields, then passed with 33 focused tests after the email-local/OID-central cutover and pre-provider guards.
  - Automated verification and authenticated native read smoke are recorded below once the full IN-476 battery completes. A production PUT remains separately approval-gated.
