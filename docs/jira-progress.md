...
- [ ] IN-378 — Retrieve meeting-scoped voiceprints for speaker identification
  ...
  - TDD evidence: route-policy assertions first failed until audio/retry were authorized for Storage API identity; client tests first failed on the absent batch models/method; resolver tests first failed on the absent service; matcher injection first failed on the absent argument; processing route/pipeline tests first failed on absent header/task propagation. Cross-repo review then caught the server's initial rich-object `missing` response versus the contract's email-string list and corrected it before release.
  - Local verification: full isolated backend suite `154 tests` passed; Storage API full suite `135 tests` plus Ruff passed; `npm run verify:storage-cutover`, `npm run verify:graph`, `npm run typecheck`, `npm run build`, contract-mirror SHA-256 comparison, and `git diff --check` all passed.
  - Release gate: neither repo branch is pushed or deployed by this implementation session. Production does not yet expose the IN-378 POST operation, so the coordinated authenticated smoke remains pending the Storage API `main` deployment. After deployment, use the exact desktop registration/scope and a read-only candidate request; never print token, OID, email, or voiceprint data. Keep the Jira item open until that smoke succeeds.
